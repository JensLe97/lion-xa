...
class DummyDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        range_img, label_2d, inpainted_mask, range_img_indices = get_range_image(
...
def preprocess(split_name, root_dir, out_dir):
    ...
    dataloader = DataLoader(DummyDataset(root_dir, split_name), num_workers=0)
...
if __name__ == '__main__':
    root_dir = '/storage/ukp/work/lemke_e/datasets/SemanticKITTI'
    out_dir = '/storage/ukp/work/lemke_e/datasets/SemanticKITTI/semantic_kitti_preprocess_lidar'
    create_pkl = True # False = only calculation
    if create_pkl:
        preprocess('val', root_dir, out_dir)
        for i in list(range(7)) + list(range(9,11)):
            preprocess('train' + str(i), root_dir, out_dir)
        preprocess('test', root_dir, out_dir)

        # data = []
        # for curr_split in ['train0', 'train1', 'train2', 'train3', 'train4', 'train5', 'train6', 'train9', 'train10']:
