...
class KNN(nn.Module):
    def __init__(self, nclasses, knn=5, search=5, sigma=1.0, cutoff=1.0):
        super().__init__()
        self.knn = knn
        self.search = search
        self.sigma = sigma
    ...
    def forward(self, proj_range, unproj_range, proj_argmax, px, py):
        ...
        # now vote (as a sum over the onehot shit)  (don't let it choose invalid)
        knn_argmax_out = knn_argmax_onehot[:, :-1].argmax(dim=1)

        # reshape again
        knn_argmax_out = knn_argmax_out.view(P)

        return knn_argmax_out, torch.squeeze(knn_argmax_onehot[:, :-1], 0).t()