...
from lion_xa.common.solver.build import (
    build_optimizer,
    build_optimizer_dis,
    build_scheduler,
    build_scheduler_dis
)
from lion_xa.models.build import (
    build_model_2d, 
    build_model_3d,
    build_model_dis_feat,
    build_model_dis_pred
)
...
from lion_xa.models.losses import entropy_loss, bce_loss
...
def train(cfg, output_dir='', run_name=''):
    ...
    # Build Discriminator for S_2D and T_3D 3D Point predictions (N, C)
    model_s2d_t3d = build_model_dis_pred(cfg)
    logger.info('Build Discriminator for S_2D and T_3D')

    # Build Discriminator for S_3D and T_2D 3D Point predictions (N, C)
    model_s3d_t2d = build_model_dis_pred(cfg)
    logger.info('Build Discriminator for S_2D and T_3D')

    # Both 2D Features
    # Build Discriminator for S_2D and T_2D 2D features (W, H, F_2D_S/T)
    model_s2d_t2d = build_model_dis_feat(cfg)
    logger.info('Build Discriminator for S_2D and T_2D')
    
    # model_2d = DataParallel(model_2d)
    # model_3d = DataParallel(model_3d)

    # model_s2d_t3d = DataParallel(model_s2d_t3d)
    # model_s3d_t2d = DataParallel(model_s3d_t2d)
    # model_s2d_t2d = DataParallel(model_s2d_t2d)

    model_2d = model_2d.cuda()
    model_3d = model_3d.cuda()

    model_s2d_t3d = model_s2d_t3d.cuda()
    model_s3d_t2d = model_s3d_t2d.cuda()
    model_s2d_t2d = model_s2d_t2d.cuda()

    # Build optimizer
    optimizer_2d = build_optimizer(cfg.OPTIMIZER_2D, model_2d)
    optimizer_3d = build_optimizer(cfg.OPTIMIZER_3D, model_3d)

    optimizer_s2d_t3d = build_optimizer_dis(cfg, model_s2d_t3d)
    optimizer_s3d_t2d = build_optimizer_dis(cfg, model_s3d_t2d)
    optimizer_s2d_t2d = build_optimizer_dis(cfg, model_s2d_t2d)

    # Build lr scheduler
    scheduler_2d = build_scheduler(cfg, optimizer_2d)
    scheduler_3d = build_scheduler(cfg, optimizer_3d)

    scheduler_s2d_t3d = build_scheduler_dis(cfg, optimizer_s2d_t3d)
    scheduler_s3d_t2d = build_scheduler_dis(cfg, optimizer_s3d_t2d)
    scheduler_s2d_t2d = build_scheduler_dis(cfg, optimizer_s2d_t2d)
    ...
    checkpointer_s2d_t3d = CheckpointerV2(model_s2d_t3d,
                                     optimizer=optimizer_s2d_t3d,
                                     scheduler=scheduler_s2d_t3d,
                                     save_dir=output_dir,
                                     logger=logger,
                                     postfix='_s2d_t3d',
                                     max_to_keep=cfg.TRAIN.MAX_TO_KEEP)
    checkpoint_data_s2d_t3d = checkpointer_s2d_t3d.load(cfg.RESUME_PATH, resume=cfg.AUTO_RESUME, resume_states=cfg.RESUME_STATES)
    checkpointer_s3d_t2d = CheckpointerV2(model_s3d_t2d,
                                     optimizer=optimizer_s3d_t2d,
                                     scheduler=scheduler_s3d_t2d,
                                     save_dir=output_dir,
                                     logger=logger,
                                     postfix='_s3d_t2d',
                                     max_to_keep=cfg.TRAIN.MAX_TO_KEEP)
    checkpoint_data_s3d_t2d = checkpointer_s3d_t2d.load(cfg.RESUME_PATH, resume=cfg.AUTO_RESUME, resume_states=cfg.RESUME_STATES)
    checkpointer_s2d_t2d = CheckpointerV2(model_s2d_t2d,
                                     optimizer=optimizer_s2d_t2d,
                                     scheduler=scheduler_s2d_t2d,
                                     save_dir=output_dir,
                                     logger=logger,
                                     postfix='_s2d_t2d',
                                     max_to_keep=cfg.TRAIN.MAX_TO_KEEP)
    checkpoint_data_s2d_t2d = checkpointer_s2d_t2d.load(cfg.RESUME_PATH, resume=cfg.AUTO_RESUME, resume_states=cfg.RESUME_STATES)
    ...
    for iteration in range(start_iteration, max_iteration):
        ...
        optimizer_s2d_t3d.zero_grad()
        optimizer_s3d_t2d.zero_grad()
        optimizer_s2d_t2d.zero_grad()
        ...
        preds_2d_tgl_feats = preds_2d['feats_2d']
        preds_2d_tgl_logit = preds_2d['point_seg_logit']
        preds_3d_tgl       = preds_3d['seg_logit']
        ...
        #   Fool the discriminator with target samples
        #   Adversarial objective to update the Segmentors 
        #   = Generator loss
        # ---------------- Train Generators ----------------
        for param in model_s2d_t3d.parameters():
            param.requires_grad = False
        for param in model_s3d_t2d.parameters():
            param.requires_grad = False
        for param in model_s2d_t2d.parameters():
            param.requires_grad = False

        # Discriminator 1 gets 2d target pred as input and source label
        d_target_2d_pred = model_s3d_t2d(F.softmax(preds_2d['point_seg_logit'].detach(), dim=1))
        loss_adv_target_2d_pred = bce_loss(d_target_2d_pred, source_label)        
        loss_adv_target_2d_pred = cfg.TRAIN.LION_XA.lambda_G_trg_2d_pred * loss_adv_target_2d_pred
        train_metric_logger.update(G_adv_trg_2d_pred=loss_adv_target_2d_pred)
        
        # Discriminator 2 gets 3d target pred as input and source label
        d_target_3d_pred = model_s2d_t3d(F.softmax(preds_3d['seg_logit'].detach(), dim=1))
        loss_adv_target_3d_pred = bce_loss(d_target_3d_pred, source_label)
        loss_adv_target_3d_pred = cfg.TRAIN.LION_XA.lambda_G_trg_3d_pred * loss_adv_target_3d_pred
        train_metric_logger.update(G_adv_trg_3d_pred=loss_adv_target_3d_pred)
        
        # Discriminator 3 gets 2d target features as input and source label
        d_target_2d_feat = model_s2d_t2d(preds_2d['feats_2d'].detach())
        loss_adv_trg_2d_feat = bce_loss(d_target_2d_feat, source_label) # Generator tries to minimize
        loss_adv_trg_2d_feat = cfg.TRAIN.LION_XA.lambda_G_trg_2d_feat * loss_adv_trg_2d_feat
        train_metric_logger.update(G_adv_trg_2d_feat=loss_adv_trg_2d_feat)
        
        loss_adv_trg = loss_adv_target_2d_pred + loss_adv_target_3d_pred + loss_adv_trg_2d_feat
        ...
        # Discriminator 1 gets 2d target-like pred as input and source label
        d_source_2d_pred = model_s3d_t2d(F.softmax(preds_2d_tgl_logit.detach(), dim=1))
        loss_adv_source_2d_pred = bce_loss(d_source_2d_pred, source_label)
        loss_adv_source_2d_pred = cfg.TRAIN.LION_XA.lambda_D_src_2d_pred * loss_adv_source_2d_pred
        train_metric_logger.update(D_adv_src_2d_pred=loss_adv_source_2d_pred)

        # Discriminator 2 gets 3d target-like pred as input and source label
        d_source_3d_pred = model_s2d_t3d(F.softmax(preds_3d_tgl.detach(), dim=1))
        loss_adv_source_3d_pred = bce_loss(d_source_3d_pred, source_label)
        loss_adv_source_3d_pred = cfg.TRAIN.LION_XA.lambda_D_src_3d_pred * loss_adv_source_3d_pred
        train_metric_logger.update(D_adv_src_3d_pred=loss_adv_source_3d_pred)

        # Discriminator 3 gets 2d target-like features as input and source label
        d_source_2d_feat = model_s2d_t2d(preds_2d_tgl_feats.detach())
        loss_adv_source_2d_feat = bce_loss(d_source_2d_feat, source_label)
        loss_adv_source_2d_feat = cfg.TRAIN.LION_XA.lambda_D_src_2d_feat * loss_adv_source_2d_feat
        train_metric_logger.update(D_adv_src_2d_feat=loss_adv_source_2d_feat)
        
        loss_adv_src = loss_adv_source_2d_pred + loss_adv_source_3d_pred + loss_adv_source_2d_feat
        loss_adv_src.backward(retain_graph=True)

        # --- Train discriminator networks with target samples --- 
        #   = Discriminator loss on target
        # Discriminator 1 gets 3d target pred as input and target label
        d_target_3d_pred = model_s3d_t2d(F.softmax(preds_3d['seg_logit'].detach(), dim=1))
        loss_adv_target_3d_pred = bce_loss(d_target_3d_pred, target_label)
        loss_adv_target_3d_pred = cfg.TRAIN.LION_XA.lambda_D_trg_3d_pred * loss_adv_target_3d_pred
        train_metric_logger.update(D_adv_trg_3d_pred=loss_adv_target_3d_pred)

        # Discriminator 2 gets 2d target pred as input and target label
        d_target_2d_pred = model_s2d_t3d(F.softmax(preds_2d['point_seg_logit'].detach(), dim=1))
        loss_adv_target_2d_pred = bce_loss(d_target_2d_pred, target_label)
        loss_adv_target_2d_pred = cfg.TRAIN.LION_XA.lambda_D_trg_2d_pred * loss_adv_target_2d_pred
        train_metric_logger.update(D_adv_trg_2d_pred=loss_adv_target_2d_pred)

        # Discriminator 3 gets 2d target features as input and target label
        d_target_2d_feat = model_s2d_t2d(preds_2d['feats_2d'].detach())
        loss_adv_target_2d_feat = bce_loss(d_target_2d_feat, target_label)
        loss_adv_target_2d_feat = cfg.TRAIN.LION_XA.lambda_D_trg_2d_feat * loss_adv_target_2d_feat
        train_metric_logger.update(D_adv_trg_2d_feat=loss_adv_target_2d_feat)
        
        loss_adv_trg = loss_adv_target_3d_pred + loss_adv_target_2d_pred + loss_adv_target_2d_feat
        loss_adv_trg.backward()

        optimizer_s2d_t3d.step()
        optimizer_s3d_t2d.step()
        optimizer_s2d_t2d.step()

        batch_time = time.time() - end
        train_metric_logger.update(time=batch_time, data=data_time)
        train_metric_logger.update(dis_optim_lr=optimizer_s2d_t3d.param_groups[0]['lr'])
        ...
        if (ckpt_period > 0 and cur_iter % ckpt_period == 0) or cur_iter == max_iteration:
            ...
            checkpoint_data_s2d_t3d['iteration'] = cur_iter
            checkpointer_s2d_t3d.save('model_s2d_t3d_{:06d}'.format(cur_iter), **checkpoint_data_s2d_t3d)
            checkpoint_data_s3d_t2d['iteration'] = cur_iter
            checkpointer_s3d_t2d.save('model_s3d_t2d_{:06d}'.format(cur_iter), **checkpoint_data_s3d_t2d)
            checkpoint_data_s2d_t2d['iteration'] = cur_iter
            checkpointer_s2d_t2d.save('model_s2d_t2d_{:06d}'.format(cur_iter), **checkpoint_data_s2d_t2d)
        ...
        scheduler_s2d_t3d.step()
        scheduler_s3d_t2d.step()
        scheduler_s2d_t2d.step()
